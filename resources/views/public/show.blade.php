        ...
        <div class="grid md:grid-cols-2 gap-6 p-6 items-start">
            ...
            <div class="bg-gray-100 rounded-lg flex items-center justify-center h-72 overflow-hidden">
                <img src="{{ str_starts_with($event->banner_path, 'images/')
                    ...
                    alt="{{ $event->title }}" class="max-h-72 max-w-full object-contain rounded">
            </div>
                    ...
                    @auth
                        @if (auth()->user()->role === 'participant')
                            @if ($event->participants_count >= $event->capacity)
                                <button disabled
                                    class="inline-flex items-center justify-center bg-gray-400 !text-white font-semibold px-4 py-2 rounded shadow cursor-not-allowed">
                                    Vagas esgotadas
                                </button>
                            @else
